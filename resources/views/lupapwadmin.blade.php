...
<body class="bg-gray-50 flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-extrabold text-gray-900 tracking-tight">Pemulihan Password</h1>
        ...
        <div id="form-card" class="bg-white rounded-3xl p-6 md:p-8 shadow-xl shadow-gray-200/50 border border-gray-100 block">
            ...
            {{-- Alert Error Bawaan Laravel (Jika Email Tidak Terdaftar) --}}
            @if ($errors->any())
            ...
            {{-- Alert Status Sukses Bawaan Laravel (Jika Menggunakan Package Auth) --}}
            @if (session('status'))
            ...
            <form action="{{ Route::has('password.email') ? route('password.email') : '#' }}" method="POST" onsubmit="handleFormSubmit(event)" class="space-y-5">
                @csrf
                ...
                <div class="text-center pt-2">
                    <a href="{{ Route::has('login-admin') ? route('login-admin') : '#' }}" class="text-xs font-bold text-gray-500 hover:text-amber-600 transition flex items-center justify-center gap-1">
        ...
        <div id="success-card" class="bg-white rounded-3xl p-6 md:p-8 shadow-xl shadow-gray-200/50 border border-gray-100 hidden text-center space-y-5">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-2 leading-relaxed">
                    Kami telah mengirimkan tautan pemulihan password ke email yang Anda masukkan. Silakan periksa folder kotak masuk atau spam.
                </p>
            </div>
            <div class="pt-2">
                <a href="{{ Route::has('login-admin') ? route('login-admin') : '#' }}" class="inline-block w-full py-3 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl text-sm transition">
                    Kembali ke Login
                </a>
            </div>
        </div>
    ...
    <script>
        function handleFormSubmit(event) {
            // Cek jika route Laravel bawaan tidak ada, maka lakukan simulasi UI murni
            const formAction = event.target.getAttribute('action');
            if (formAction === '#') {
                event.preventDefault(); // Gagalkan submit real ke backend
                
                // Transisi UI dari Form Input ke Tampilan Sukses
