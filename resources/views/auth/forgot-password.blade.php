...
<head>
    ...
    <title>Lupa Password - KSPM</title>
...
<body class="bg-gray-50 flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-md">
        ...
        <div id="form-card" class="bg-white rounded-3xl p-6 md:p-8 shadow-xl shadow-gray-200/50 border border-gray-100 block">
            ...
            <form action="{{ route('user.forgot-password.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="text-center pt-2">
                    <a href="{{ route('login') }}" class="text-xs font-bold text-gray-500 hover:text-amber-600 transition flex items-center justify-center gap-1">
                        <span>⬅️</span> Kembali ke Login
                    </a>
                </div>
            </form>
        </div>
        ...
        <div id="success-card" class="bg-white rounded-3xl p-6 md:p-8 shadow-xl shadow-gray-200/50 border border-gray-100 hidden text-center space-y-5">
            ...
            <div class="pt-2">
                <a href="{{ route('login') }}" class="inline-block w-full py-3 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl text-sm transition">
                    Kembali ke Login
                </a>
            </div>
        </div>
    ...
    <script>
        // Otomatis pindah ke screen sukses jika backend mengirimkan session status berhasil
        @if (session('status'))
